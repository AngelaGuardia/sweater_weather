...
describe Roadtrip do
  ...
  it "can set the weather" do
    VCR.use_cassette('denver_pueblo_roadtrip') do
      ...
      weather_at_eta = roadtrip.set_weather({ roadtrip: roadtrip_info, travel: travel_info, forecast: forecast})

      expect(weather_at_eta).to be_a(Hash)

      expect(weather_at_eta[:temperature]).to eq(39.02)
      expect(weather_at_eta[:conditions]).to eq("few clouds")
    end
  end
end
